...
===============================
 ...
 Writer: Josef Schulz
 File: fmAllocateHierarchy.cpp
===============================
*/
#include <FatalisMatrix.h>

HRESULT fmAllocateHierarchy::CreateFrame(LPCTSTR Name, LPD3DXFRAME *ppNewFrame)
{    
	// Create a frame
...
long fmAllocateHierarchy::CreateMeshContainer(
		LPCTSTR Name, 
...
HRESULT fmAllocateHierarchy::DestroyFrame(LPD3DXFRAME pFrameToFree) 
{
	//Convert the frame
	LPFRAME pFrame = (LPFRAME)pFrameToFree;

	// Delete the name
	FM_SAFE_DELETE_ARRAY(pFrame->Name);
	
	// Delete the frame
    FM_SAFE_DELETE(pFrame);

	// Returns an HRESULT so give it the AOk result
    return S_OK; 
}

HRESULT fmAllocateHierarchy::DestroyMeshContainer(LPD3DXMESHCONTAINER pMeshContainerBase)
{
	//Convert to my derived struct type
    LPMESHCONTAINER pMeshContainer = (LPMESHCONTAINER)pMeshContainerBase;
	
	// if there is a name
	FM_SAFE_DELETE_ARRAY(pMeshContainer->Name);

	//if there are materials
	FM_SAFE_DELETE_ARRAY(pMeshContainer->pMaterials9);

	//Release the textures
	if(pMeshContainer->ppTextures)
		for(UINT i = 0; i < pMeshContainer->NumMaterials; ++i)
				FM_SAFE_RELEASE(pMeshContainer->ppTextures[i]);

	//if there are textures
	FM_SAFE_DELETE_ARRAY(pMeshContainer->ppTextures);

	// if there is adjacency data
	FM_SAFE_DELETE_ARRAY(pMeshContainer->pAdjacency); 
	
	// if there are bone parts
	FM_SAFE_DELETE_ARRAY(pMeshContainer->pBoneOffsets);
	
	//if there are frame matrices
	FM_SAFE_DELETE_ARRAY(pMeshContainer->ppFrameMatrices);
	
	FM_SAFE_DELETE_ARRAY(pMeshContainer->pAttributeTable);
	
	//if there is a copy of the mesh here
	FM_SAFE_RELEASE(pMeshContainer->pSkinMesh);
	
	//if there is a mesh
	FM_SAFE_RELEASE(pMeshContainer->MeshData.pMesh);
	
	// if there is skin information
	FM_SAFE_RELEASE(pMeshContainer->pSkinInfo);
	
	//Delete the mesh container
	FM_SAFE_DELETE(pMeshContainer);
