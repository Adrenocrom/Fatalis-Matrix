...
fmSkin::~fmSkin()
{
	...
	FM_SAFE_RELEASE(m_pAnimController);
	
	//if there is a frame hierarchyo
	if(m_pFrameRoot)
	{
		//Allocation class
		fmAllocateHierarchy Alloc;
		D3DXFrameDestroy(m_pFrameRoot, &Alloc);
		m_pFrameRoot = NULL;
	}

	//Delete the bones
	FM_SAFE_DELETE_ARRAY(m_pBoneMatrices);
}
...
void fmSkin::LoadXFile(char* strFileName)
{
...
void fmSkin::SetupBoneMatrices(LPFRAME pFrame, LPD3DXMATRIX pParentMatrix)
{
	fmDirect3D& D3D = fmDirect3D::Instance();
	...
	if(pMesh)
	{
		...
		if(pMesh->pSkinInfo)
		{
			...
			pMesh->MeshData.pMesh->CloneMeshFVF(D3DXMESH_MANAGED, 
				pMesh->MeshData.pMesh->GetFVF(), D3D.GetDevice(), 
				&pMesh->pSkinMesh);
...
void fmSkin::SetCurrentAnimation(DWORD dwAnimationFlag)
{
...
void fmSkin::Draw()
{
	...
	while(pMesh)
	{
		...
		{
			D3D->SetMaterial(&pMesh->pMaterials9[i]);
			D3D->SetTexture(0, pMesh->ppTextures[i]);
...
void fmSkin::DrawFrame(LPFRAME pFrame)
{
	fmDirect3D& D3D = fmDirect3D::Instance();
...
void fmSkin::Update(double dElapsedTime)
{
...
void fmSkin::UpdateFrameMatrices(LPFRAME pFrame, LPD3DXMATRIX pParentMatrix)
{	
	//Parent check
	if (pParentMatrix)
	{
