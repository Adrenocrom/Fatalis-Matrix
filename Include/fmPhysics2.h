	...
	void Move(float fTimeStep, float fBouyanceFactor)
	{
		...
		if(m_fRadius == 0.0f)
		{
			if(m_vWorldPos.y > g_fSealevel)
				vAcceleration = m_vForce  + g_vGravity + (g_vBuoyanceAir * fBouyanceFactor);
			else
				vAcceleration = m_vForce  + g_vGravity + (g_vBuoyanceWater * fBouyanceFactor);
		}
		else
		{
			if(m_vWorldPos.y > g_fSealevel)
...
public:
	...
	void SetTransformation(float fNumSecsPassed)
	{
		...
		m_mTransformation = mTranslation * mRotation;
	}
};
